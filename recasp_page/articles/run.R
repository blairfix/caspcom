...
dir = here()
dir = paste(dir, "recasp_page/articles", sep = "/")
...
for(path in f){
  ...
  table_entry = table_temp
    ...
    paper_out = string_replace("title-here", title, paper_out)
    paper_out = string_replace("subtitle", subtitle, paper_out)
    
    recasp_authors = toupper(author)
    recasp_authors = string_replace(" AND", " and", recasp_authors)
    paper_out = string_replace("authors", recasp_authors, paper_out)
    
    paper_out = string_replace("date", date, paper_out)
...
banner = '<img src="https://capitalaspower.com/wp-content/uploads/2021/04/recasp_banner.png" />'
heading = '<h1 style="font-weight: bold;">Articles</h1>'

page_out = c(header, banner, heading, page_out, "</body></html>")
